...
<script type="text/ecmascript-6">
  ...
  import { inBookShelf, addOrRemoveShelf } from '../../utils/store'
  ...
  export default {
    ...
    computed: {
      ...
      inBookShelf() {
        ...
        if (inBookShelf(this.bookItem, this.shelfList)) {
          return true
        } else {
          return false
        }
    ...
    methods: {
      addOrRemoveShelf() {
        const bookList = this.shelfList
        addOrRemoveShelf(this.inBookShelf, this.bookItem, bookList)
        this.setShelfList(getBookShelf())
      },
      // 展示Toast弹窗
      showToast(text) {
